...
class FastFlowPipeline:
    ...
    def setup(self):
        ...
        if self.config.get("model", "fastflow") == "ssn":
            print(f"\nInitializing SuperSimpleNet model (backbone={self.config['backbone']})")
            # In unsupervised mode, SSN uses stop_grad=True (matches anomalib)
    ...
    def evaluate(self):
        ...
        if self.config.get("model", "fastflow") == "ssn":
            self.evaluator = SuperSimpleNetEvaluator(
                model=self.model,
                device=str(self.device)
            )
        else:
...
def parse_args():
    ...
    parser.add_argument('--perlin_threshold', type=float, default=0.2,
                        help='SSN: threshold for smooth-noise mask (anomalib default 0.2)')
    parser.add_argument('--adapt_cls_features', action='store_true',
                        help='SSN: adapt classification features too (ICPR mode); default False (JIMS extension)')
