...
class FastFlowTrainer:
    ...
    def __init__(
        ...
        monitor: str = "val_loss",      # leakage-free selection on held-out normals
        maximize: bool = False,         # val NLL: lower is better
    ):
        self.model = model.to(device)
        self.device = device
        self.backbone_name = backbone_name

        # save_dir already encodes category/model/backbone from the caller; do not
        # nest again so checkpoints and metrics.json live in the same folder.
    ...
    def fit(
        ...
        for epoch in range(num_epochs):
            ...
            # Selection signal: NLL on held-out normal images. No test data is
            # used here, so the chosen checkpoint is not tuned on the test set.
            ...
            if np.isfinite(current) and improved:
                ...
                patience_counter = 0
                print(f"✓ New best model saved (val_loss={best_metric:.4f})")
            else:
                patience_counter += 1
...
class FastFlowEvaluator:
    ...
    def compute_metrics(self, predictions: Dict[str, np.ndarray]) -> Dict[str, float]:
        ...
        # Standard pixel AUROC: every pixel of every test image counts, so normal
        # images contribute true negatives (using has_mask>0 only would measure
        # separation *within* defective images and inflate the score).
        pixel_labels = (masks.reshape(-1) > 0.5).astype(np.int64)
